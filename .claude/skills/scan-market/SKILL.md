...
4. **L4 研究(token 大头,一只=一个 Opus subagent)**——helper 在 `autoresearch.scan.agents.l4_card`:
   ...
   - **买单 skeptic · ≥OW · 独立 Opus**:`l4_card.pick_buy_candidates(ratings)`(最终 Buy/OW)每只派一个**独立** `Agent(subagent_type='buy-skeptic')` 证伪(人设/攻击面/落稿契约已烤进 `.claude/agents/buy-skeptic.md`;模式 A=空方,自动落 `_v_<code>.md`),主线当 **PM 用 3 透镜投票裁判** → `verify.csv`(`code,verdict,bull,bear,trigger,consensus`)。落定后 `watchlist.ingest_verify(scan_dir, 'context/watchlist.csv')` 把**降级**条目草拟进观察单,并由编排层(你)为其补**结构化 conds**(词表 v2:`close_above/close_below/ma_bull/money_pos/by_date(日期锚⏰)+manual`,与 L4 卡触发叙事对齐)——观察单从此每天被 `run_check` 机判。
