...
1. **取数（零 LLM）**：
   ```bash
   uv run python -m autoresearch.analyze.harvest TICKER [YYYY-MM-DD] [stock|crypto] [PEER1,PEER2,...]
   ```
...
5. **组装+校验**：
   ```bash
   uv run python -m autoresearch.analyze.assemble context/analyze/<TICKER>_<分析日YYYYMMDD> [--name <A股中文简称>]
   ```
...
- **(v4 A股)** **股东户数=散户数量**：户数↓=筹码集中(偏多)、↑=分散(高位放量增加=派发嫌疑)，**看趋势而非单期**；解禁日历只列**未来**供给压力(过去的已消化)；akshare 列名跨版本会变，已防御性取列，缺则 WebSearch 兜底；股权质押率走 tushare(`pledge_stat`，>40%=爆雷红旗)、WebSearch 兜底。**主力资金流要落『逐日表』(近5日)并读 day-by-day 模式(涨停日大额流入→随后连续净流出=拉高出货)，别只报累计数；harvest 现 tushare-first(逐日资金/真技术·多头排列·RSI·MACD/筹码/北向一把取),失败才回退 push2/WebSearch 补回逐日颗粒度,别静默降成单个新闻数。实证(factor_lab):量比高≠强势(放量滞涨)、主力净流入是 1–2 周 swing 信号(非次日)、低筹码获利比例**非**超跌买点。**
