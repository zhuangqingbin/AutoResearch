...
# ann_date) 唯一组合只有 218 个;`ev_holder_in>1` 的票里 75.8%(25/33)只有 1 个 `ann_date`。榜首
...
def market_event_counts(date: str, *, lookback_days: int = 10, fetch_fn=None) -> pd.DataFrame:
    ...
    # `ok_by_ep['stk_surv']=5`,`n==0` 不成立,一个字都不打。文档说假话比没文档更坏:
    # 下一个读者会以为这情形已被守住而跳过真正该建的守卫):
    #   ① n == 0:整窗口零产出 = 腿死了;
    #   ② 0 < n < 窗口一半:大面积缺日(含"缺最新一日 + 还缺一半"这种严重形态)——advisory
    #      档,只记账不改行为。**边界照实说**:5/10 这种缺日(= 07-21 stk_surv 的实况,
    #      连续 20260708/09/10/13/14 五个交易日空)仍然不打字,是**有意**的取舍(腿没死,
    #      天天告警就是噪声),不是被覆盖了——但这恰好卡在 `n==half` 触发边界、**零余量**
    #      (再空 1 天 n=4 就会触发 ② 档告警。终审 I-2 复核,2026-07-25:原注释的
    #      "7/10" 系笔误,现值已用真湖数据独立重跑二次核对)。要守"缺最新一日"本身,
    #      得另建一条按日历逐日对账的守卫,本函数不声称提供。这 5 个空文件是否为真实的
    #      "当日无调研"还是湖内钉死的空快照
    #      (同族前科:lake-narrow-fields-poisoning)是独立待查项,见 progress.md Wave4 段。
