...
## 冒烟实录(2026-07-25·控制端自跑)

**Step 1 全市场事件计数(真湖)**:`market_event_counts('2026-07-21')` → **566 只**有事件票(`ev_hard>0` 325 只 / `ev_pos>0` 448 只);三端点湖命中,`stk_surv` 有若干日真实空(B 级合法空,已留痕)。
按 `ev_hard` 降序 top8 **全部是真公司行为**(回购实施/预案/增持,max=3)——Task 2 review 逮到的"调研人次霸榜"(原 max=82,top10 全是纯调研)已治好。

**Step 2 event 路召回**:quota=80 → 召回 80 只,其中现有 9 路**没召到的 unique = 40 只**(边际贡献面 50%)。

**Step 3 前瞻性回看(n=1 日,不裁决,照实记)**:

| 口径 | n | fwd_2_oc | 对市场超额 | t | p |
|---|---|---|---|---|---|
| 全市场 | 5524 | +1.61% | — | — | — |
| `ev_hard>0`(会员面) | 325 | +1.65% | +0.05pp | +0.21 | 0.833 |
| `ev_pos>0` | 448 | +1.61% | +0.00pp | +0.00 | 0.996 |
| `ev_rep_impl>0` | 81 | +1.57% | −0.03pp | −0.09 | 0.927 |
| `ev_holder_in>0` | 67 | +1.78% | +0.18pp | +0.38 | 0.705 |
| `ev_surv_n>0` | 133 | +1.49% | −0.12pp | −0.28 | 0.783 |
| **event 路召回 80 只** | 80 | +0.79% | **−0.82pp** | **−2.01** | **0.048** |
| **其中 unique 40 只**(= `unique_excess_t2` 口径) | 40 | +0.60% | **−1.01pp** | −1.83 | 0.075 |

**诚实读数**:会员面**零 edge**(全部 |t|<0.4);**边际面为负**且在 5% 水平显著。这正是当年裁掉 `accumulation` 用的同一指标(它 13 日累计 −0.21%)。**n=1 日不构成裁决**(纪律:≥10 数据日),但第一读明确不利——按 [[l3-no-selection-alpha-20260712]] 的方法论,**先记账、不辩护、不提前松门**。若十日累计维持这个方向,`event` 路应直接退役而非启用,本波的价值就落在①仪器修好了(能做出这个判断)②新闻腿断链变可见③全市场事件计数可供别处复用。
