...
- **(v4 A股)** **股东户数=散户数量**：户数↓=筹码集中(偏多)、↑=分散(高位放量增加=派发嫌疑)，**看趋势而非单期**；解禁日历只列**未来**供给压力(过去的已消化)；akshare 列名跨版本会变，已防御性取列，缺则 WebSearch 兜底；股权质押率走 WebSearch。**主力资金流要落『逐日表』(近5日)并读 day-by-day 模式(涨停日大额流入→随后连续净流出=拉高出货)，别只报累计数；东财资金流接口失败时 WebSearch 补回逐日颗粒度，别静默降成单个新闻数。**
