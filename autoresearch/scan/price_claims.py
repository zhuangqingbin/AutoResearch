...
**语义边界(2026-07-23 终审 C-1 收紧,三类真卡假阳):抽取器只认领「某日已实现的单日
股价移动」。**三类被排除(否则对账器会对读者输出「分析师捏造股价」的自信误指控):
  1. 区间/累计移动(`A日→B日 涨X%`):日期与 % 之间出现区间标记(→/至/到/从)⇒ 弃
     (华海 600521:`本股 6/09 14.64→7/16 17.63(+20%)` 是累计 +20%,非 6/09 单日)。
  ...
  3. 基本面/非股价 %:% 前后 8 字内出现基本面名词(营收/收入/净利/利润/毛利/EPS/业绩/订单/
     产能/份额/同比/环比)⇒ 弃(协创 300857:`营收同比上涨12%` 是营收%非股价%)。

**已知简化(终审遗留#1):每句只取首个可认领断言(under-report,非假阳)——上面三类先滤掉
...
def _is_realized_price_pct(sent: str, dm: re.Match, pm: re.Match, dates: list[re.Match]) -> bool:
    """pm(% 匹配)配 dm(其最近在前日期)是否为一条「已实现单日股价移动」。四类排除:
      · 区间幻影(如"5-10%"):_DATE 把 "5-10" 当日期、_PCT 把 "-10%" 当字面负号,两匹配抢同段字符;
      · 区间标记(→/至/到/从)落在日期簇与数字之间 → 累计/区间移动非单日(华海 6/09→7/16 +20%);
      · 数字之前局部窗有情景/目标语境 → 前向情景 EV/目标价(普冉 延续至510 +8.5%);
      · 数字前后 8 字内有基本面名词 → 营收/净利% 非股价%(协创 营收同比 +12%)。"""
