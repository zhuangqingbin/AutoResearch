...
> **实测(2024-06~2026-06 全网格,OOS 落在近 2026-Q2)**:全 7 个 core 模型(linear/lgbm/xgb/cat/double/mlp/tabnet)OOS rank-IC **仍为负**(xgb 最不负但仍 <0)→ **无晋升、L2 回落 composite**。诚实结论:**近窗因子整体反预测,光扩样本不够**——要拿到正-IC champion 需补**更好特征**(如递延的季度基本面)或 **regime-aware 建模**,非本期范围。L2 的价值此刻在"宽召回 + 不部署反预测模型",真正的 alpha 交给 L3(Claude holistic)/L4(深研)。
