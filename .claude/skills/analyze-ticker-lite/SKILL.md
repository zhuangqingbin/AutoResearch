...
description: Use when you need a FAST, low-token decision card for a single ticker (5-tier rating + 3-scenario target/R:R + variant view + tripwires) instead of a full deep-dive report — chiefly as scan-market's L3b workhorse over the ~30 finalists, or when the user wants a quick read on one name. ~20–30% of analyze-ticker's tokens. For the full evidence-appendix report use analyze-ticker. Project-local.
---

# analyze-ticker-lite — 单只决策卡(轻量,~20–30% token)

## 核心原理
和 analyze-ticker 同源(同一免费数据层 + Claude 当引擎),但**只产出一张决策卡**,且 harvest 走 `--slim`(只取决策驱动块)。省 token 的两个杠杆:**harvest 更少(输入)+ 只写卡(输出)**。用于 scan-market 的 L3b(~30 只 finalists 逐只跑),或用户想快速判一只票。

**与 analyze-ticker 的分工**:lite 出"买不买"的卡;若某只值得下重注,再对它单独跑**全量 analyze-ticker** 看完整证据附录。

## 何时用 / 不用
- ✅ scan-market L3b:对 finalists 批量出决策卡。
...
2. **写决策卡**:读 slim context,按 `lite-playbook.md` 产出**单文件** `reports/<date>/<ticker>/complete_report.md`(一张卡)。**每个数字出自 context**;沿用 analyze-ticker 的数据坑/铁律(见其 `engine-playbook.md`)。
