...
- **L4 委托 stock-research(lite 档)**:**一只 finalist = 一个 Opus subagent 渐进深度 DD + 早停**(P0 简报定向 → P1–P3 表面 → 主早停② → P4 陷阱核 → ③击杀 → P5 满卡;早停只向下、≥OW 必走 P4+P5)→ 买单(≥OW)独立 Opus skeptic 证伪。
...
## 流程(6 段)
> 操作模板分驻各能力 skill(2026-07-03 海拔重构):市场研判在 `macro-research/macro-playbook.md` 末节、L4 决策卡在 stock-research 的 `lite-playbook.md`;L3 rubric / skeptic / 红队要点内联下方步骤与 `STAGES.md`;**各阶段现状快照**(引擎/参数/实证读数/已知局限/勿重启清单)见 `STAGES.md`(as-of 日期标注,冲突以源码为准)。screening-playbook 已退役(历史在 git)。

0. **⚡ 前奏一键(推荐,取代下方 1/2 的人肉串)**:
   ```bash
   uv run --no-sync python -m autoresearch.scan.prelude <YYYY-MM-DD>
   ```
   一条命令跑完全部确定性前奏:attribution 刷新(成熟老日 fwd 补齐)→ retro pending 列出(**只备料,诊断仍走 scan-retro**)→ consensus 拉 → universe(regime-aware+影子)→ 日历 → 观察单日检(**🔔 触发置顶警报**)→ 菜单/L4 预算/哨兵建议 → journal+buy_ledger+cross_calib 刷新。各步失败不阻断,末尾汇总屏(含 **📐/🔁/🚪 当日件建议行**:📐 触价校准贴 `_l4_shared_instructions.md`、🔁 L3 翻案贴 L3 校准块旁、🚪 门柱贴 skeptic/PM 先验;**含「禁注」的行勿贴**)。之后从 2.2 哨兵决策接 LLM 段(market_view 已由 0.5 并发产出;未产则按 2.5 补)。下方 1–2 保留作分解说明与调参入口。
...
4. **L4 研究(token 大头,一只=一个 Opus subagent)**——helper 在 `autoresearch.scan.agents.l4_card`:
   ...
   - **买单 skeptic · ≥OW · 独立 Opus**:`l4_card.pick_buy_candidates(ratings)`(最终 Buy/OW)每只派一个**独立** `Agent(subagent_type='buy-skeptic')` 证伪(人设/攻击面/落稿契约已烤进 `.claude/agents/buy-skeptic.md`;模式 A=空方,自动落 `_v_<code>.md`),主线当 **PM 用 3 透镜投票裁判** → `verify.csv`(`code,verdict,bull,bear,trigger,consensus`)。落定后 `watchlist.ingest_verify(scan_dir, 'context/watchlist.csv')` 把**降级**条目草拟进观察单,并由编排层(你)为其补**结构化 conds**(词表 v1:`close_above/close_below/ma_bull/money_pos/manual`,与 L4 卡触发叙事对齐)——观察单从此每天被 `run_check` 机判。
   - **机会成本红队 · 仅 0 买日 · 独立 Opus**(对称性修复:空仓也要红队):verify 折回后今日无 ≥OW → `l4_card.pick_opportunity_candidates(ratings, scan_dir, k=2)`(conviction 最高的 Hold top-2)每只派一个独立 `Agent(subagent_type='buy-skeptic')`(**模式 B=红队多方**,prompt 注明)攻"压评级的那道门",PM 三透镜裁判。**产出只进观察单(结构化 conds,source=opp_redteam)与校准数据,不改评级**——门的松紧不动,但系统从此有"门是否太紧"的对抗证据流(prompt 要点见 `STAGES.md`『买单 skeptic + 机会成本红队』节:人设=错过成本审计员、攻 binding gate、翻转触发用观察单词表、不改评级不喊单)。
5. **L5 整合**:
   ```bash
   uv run --no-sync python -m autoresearch.scan.assemble <date>
   ```
   → **`reports/scan/<YYYYMMDD_HHMM>/`**(目录名 = **实际运行时刻**;数据日 analysis_date 记 `manifest.json`,解耦,retro 据此定位):`summary.md`(三段:漏斗数量 / 各阶段概览 / **buy-list〔逐阶段结论表 L1→L2→L3→L4 + 买单 skeptic 徽标〕** + **🏭 行业研判 / 🔗 同链对比**〔sector_briefs 在才嵌,presence-gated;行业方向自动进 sector_ledger〕 + **各阶段 token 估算**)+ `details/〈股票名称〉.md`(决策卡按名称命名)+ `trace/`(每阶段全量数据 + `reasoning/` 留痕 + funnel)。**汇报**:漏斗 + buy-list(评级/目标 + 多空 verdict)+ 诚实局限。
