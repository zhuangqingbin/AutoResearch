...
动机:scan.universe 的四透镜权重是**作者先验**(无回测)。本工具把"分数 → 未来收益"
...
用法:
  uv run --no-sync python -m autoresearch.research.factor_lab harvest [--step 4] [--form-span 90]
  uv run --no-sync python -m autoresearch.research.factor_lab eval
  uv run --no-sync python -m autoresearch.research.factor_lab --selftest    # 离线验 IC/十分位数学
  uv run --no-sync python -m autoresearch.research.factor_lab harvest --dry # 只打印计划(日期/调用数)
"""
...
def factor_frame(D: str, piv: dict, P: list[str], basic: pd.DataFrame,
    ...
    # 硬门(与 scan.universe 一致):非 ST、市值地板、非次新、剔北交所、有量
...
# 原始因子:factor_lab.factor_frame 与 scan.universe L1_recall **都产出**的列(去掉只在一侧的)。
GBDT_RAW = [
