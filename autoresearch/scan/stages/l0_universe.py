...
**复用 screen_market 的确定性取数/门函数,绝不重写**:
  - universe:`screen_market.fetch_universe_tushare`(source=tushare,默认)或 `fetch_universe`(em)。
  - 轻召回门:`screen_market._recall_gate_a`(只去真不可交易/无核心数据的尾部)。
等价于 `screen_market.run` 的 "L0 取数 + `uni = uni[_recall_gate_a(uni)]`" 这一段——逐函数照搬,
...
def _screen_market():
    """延迟导入 scripts/screen_market(确定性漏斗的现有实现;不在 import 期拉起网络)。"""
...
class L0Universe(Stage):
    ...
    def run(self, ctx: RunContext) -> None:
        ...
        if cfg.source == "tushare":
            # 与 screen_market.run 同款:fetch_universe_tushare 是 tushare_source 的函数(screen_market
            # 在 run() 内局部 import 它,并不挂为模块属性)→ 从同一模块取,patch 点也在那。
            ...
            uni = sm.fetch_universe(
                ctx.analysis_date, cap_floor_yi=cfg.cap_floor, include_bj=cfg.include_bj)
            n_raw = sm._GATE_INFO.get("n_raw", len(uni))
        n_l0 = len(uni)

        # 轻召回门(screen_market.run 的 `uni = uni[_recall_gate_a(uni)]`)+ code 规整。
        uni = uni[sm._recall_gate_a(uni)].reset_index(drop=True)
