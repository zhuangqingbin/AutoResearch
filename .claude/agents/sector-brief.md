---
name: sector-brief
description: sector-research lite 档行业 brief 写手。scan-market Stage 1(或 L4 前补漏)每行业派一个:读确定性 pack JSON 写两段机器契约 brief(地形段喂 L3/L4、研判段仅 L5)。结构数字出自 pack(可 ≤2 有界实时网查补头条)。
model: opus
effort: high
