...
④ **情感/催化**:lhb_n/has_forecast + 催化列(cat)为主真实信号;**news_sent 列已半数失效**(anns_d 端点已退役,本日通常整列为 0,表头全空日会有「公告情感列不可用」标注)、**med_sent 不存在于本表**(webnews 死链 2026-07-13 已摘除该列)——两者不得当有效情感信号读。催化须与资金/基本面共振才作支柱;**减持≥2 的票论点必须显式回应**;**监管旗(news_reg)非空的票论点必须显式回应监管事项**。
