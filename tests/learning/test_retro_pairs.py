...
fail = 评级高(bought,或 0 买日取当日最高评级档)但 T+2 跌;
success = 同日被门拦/漏召回(bucket missed_*)但 T+2 涨(winner)。
...
def test_build_pairs_matches_same_industry():
    attr = pd.DataFrame([
        ...
        _row("000002", "漏赢", "半导体", "Underweight", 0.15, winner=True, bucket="missed_l1"),  # success 同业
        _row("000003", "漏赢2", "医药", "Sell", 0.20, winner=True, bucket="missed_l0"),
    ])
...
def test_build_pairs_zero_buy_day_uses_top_rating_present():
    ...
    attr = pd.DataFrame([
        ...
        _row("000012", "漏赢", "电子", "Underweight", 0.12, winner=True, bucket="missed_l1"),
    ])
    pairs = build_retro_pairs(attr)
    assert len(pairs) == 1
...
def test_build_pairs_empty_when_fwd2_immature():
    attr = pd.DataFrame([
        _row("000001", "a", "半导体", "Overweight", np.nan),
        _row("000002", "b", "半导体", "Underweight", np.nan, winner=False, bucket="missed_l1"),
    ])
    assert build_retro_pairs(attr).empty                                     # fwd_2 未成熟 → 优雅空


def test_build_pairs_factor_diff_is_fail_minus_win():
    attr = pd.DataFrame([
        _row("000001", "买错", "半导体", "Overweight", -0.06, composite=0.8, mom=0.5, wr=92.0),
        _row("000002", "漏赢", "半导体", "Underweight", 0.15, winner=True, bucket="missed_l1",
...
def test_build_pairs_fallback_global_when_no_same_industry():
    attr = pd.DataFrame([
        ...
        _row("000002", "漏赢", "医药", "Underweight", 0.15, winner=True, bucket="missed_l0"),
    ])
    pairs = build_retro_pairs(attr)
    assert len(pairs) == 1
